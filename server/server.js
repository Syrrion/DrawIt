...
io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', ({ username, avatar, roomCode }) => {
        ...
        rooms[roomCode].game.scores[socket.id] = 0;
        ...
        const roomState = { 
            roomCode, 
            users: rooms[roomCode].users,
            drawHistory: rooms[roomCode].drawHistory,
            layers: rooms[roomCode].layers,
            leaderId: rooms[roomCode].leaderId,
        ...
        if (rooms[roomCode].gameState === 'PLAYING') {
            roomState.game = {
                scores: rooms[roomCode].game.scores,
                currentRound: rooms[roomCode].game.currentRound,
                totalRounds: rooms[roomCode].game.totalRounds,
                currentDrawerIndex: rooms[roomCode].game.currentDrawerIndex,
                turnOrder: rooms[roomCode].game.turnOrder
            };
        }
    ...
    socket.on('playerReady', (roomCode) => {
        ...
        if (room && room.gameState === 'READY_CHECK') {
            if (!room.readyPlayers.includes(socket.id)) {
                ...
                if (room.readyPlayers.length === room.users.length) {
                    if (room.readyCheckTimer) clearTimeout(room.readyCheckTimer);
    ...
    function startActualGame(roomCode) {
        ...
        if (room.settings.mode === 'guess-word') {
            ...
            room.game.turnOrder = shuffle(room.users.map(u => u.id));
            ...
            if (targetUser) {
                ...
                if (index !== -1) {
                    ...
                    if (room.gameState === 'READY_CHECK') {
                        ...
                        if (room.readyPlayers.length === room.users.length && room.users.length > 0) {
                            if (room.readyCheckTimer) clearTimeout(room.readyCheckTimer);
    ...
    socket.on('chatMessage', ({ roomCode, username, message }) => {
        ...
        if (room.gameState === 'PLAYING' && room.settings.mode === 'guess-word' && room.game.currentWord) {
            const drawerId = room.game.turnOrder[room.game.currentDrawerIndex];
